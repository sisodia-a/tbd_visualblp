##############################################################################
# (c) Model: structured + visual in the demand, instruments from
#            structured + visual, random coefficients on [price + structured char].
##############################################################################
...
# supply_instruments = np.c_[
#     pyblp.build_blp_instruments(pyblp.Formulation('1 + log(hpwt) + log(mpg) + log(space) + bodyshape + boxiness + grille_height + grille_width'), uk_product_data),#     pyblp.build_blp_instruments(pyblp.Formulation('0 + trend'), uk_product_data)[:, 0],
#     uk_product_data['mpd'],
# ]
...
uk_initial_sigma = np.diag([1.5, 0, 8.5, 8.5, 8.5, 0, 0, 0, 0])
uk_initial_pi = np.c_[[0, -14, 0, 0, 0, 0, 0, 0, 0]]

uk_sigma_bounds = (
        np.zeros_like(uk_initial_sigma),
        np.diag([100, 0, 100, 100, 100, 0, 0, 0, 0])
)
uk_pi_bounds = (
...
counterfactual_shares.to_csv('counterfactual_shares3_c.csv',index=False)
...
visual_counterfactual_shares.to_csv('visual_counterfactual_shares3_c.csv',index=False)

uk_product_data.to_csv('exp_uk_product_data_c_3.csv',index=False)
# pd.DataFrame(uk_results_with_supply.xi_fe, columns = ['xi_fe']).to_csv('exp_xi_fe_c_3.csv',index=False)
pd.DataFrame(updated_results.xi_fe, columns = ['xi_fe']).to_csv('exp_xi_fe_c_3.csv',index=False) 
