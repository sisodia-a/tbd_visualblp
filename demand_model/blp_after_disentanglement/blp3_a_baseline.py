...
counterfactual_simulation = pyblp.Simulation(
        product_formulations = (pyblp.Formulation('1 + hpwt + mpd + space', absorb='C(clustering_ids)'),
        pyblp.Formulation('1 + prices + hpwt + mpd + space'),
        pyblp.Formulation('1 + log(hpwt) + log(mpg) + log(space) + trend')),
...
counterfactual_shares.to_csv('counterfactual_shares3_a.csv',index=False)

uk_product_data.to_csv('exp_uk_product_data_a_3.csv',index=False)
# pd.DataFrame(uk_results_with_supply.xi_fe, columns = ['xi_fe']).to_csv('exp_xi_fe_a_3.csv',index=False)
pd.DataFrame(updated_results.xi_fe, columns = ['xi_fe']).to_csv('exp_xi_fe_a_3.csv',index=False) 
