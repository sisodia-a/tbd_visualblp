...
class VAE(nn.Module):
    def __init__(self, img_size, encoder, decoder, regression, latent_dim, model_type, threshold_val,sup_signal):
        ...
        if self.sup_signal == 'make':
           self.regression = regression(self.latent_dim,28)
    ...
    def forward(self, x, make, makemodel, color, firm, region, price, hp, mpg, mpd, hpwt, space, wt, length, wid, ht, wb, xi_fe):
        """
