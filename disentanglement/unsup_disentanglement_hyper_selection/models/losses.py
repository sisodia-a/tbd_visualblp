...
class BtcvaeLoss():
    ...
    def __init__(self, record_loss_every=50, rec_dist="laplace", steps_anneal=0,n_data=1, alpha=1., beta=6., gamma=1., delta=1000.,sup_signal1="xi_fe",sup_signal2="price",sup_signal3="price",sup_signal4="price",sup_signal5="price",number_of_signals=1,is_mss=True, **kwargs):
    ...
    def __call__(self, data, recon_batch, latent_dist, is_train, storer, wtp_pred1, wtp_pred2, wtp_pred3, wtp_pred4, wtp_pred5, wtp1, wtp2, wtp3, wtp4, wtp5,
        ...
        if self.number_of_signals == 1:
            mse_loss = mse_loss1
        elif self.number_of_signals == 2:
            mse_loss = mse_loss1 + mse_loss2
        elif self.number_of_signals == 3:
            mse_loss = mse_loss1 + mse_loss2 + mse_loss3
        elif self.number_of_signals == 4:
            mse_loss = mse_loss1 + mse_loss2 + mse_loss3 + mse_loss4
        elif self.number_of_signals == 5:
            mse_loss = mse_loss1 + mse_loss2 + mse_loss3 + mse_loss4 + mse_loss5
        else:
            mse_loss = 0  # Or handle as appropriate if number_of_signals is out of expected range
