...
class BtcvaeLoss():
    ...
    def __call__(self, data, recon_batch, latent_dist, is_train, storer, wtp_pred1, wtp_pred2, wtp_pred3, wtp_pred4, wtp_pred5, wtp1, wtp2, wtp3, wtp4, wtp5,
        ...
        # wtp_pred1 = wtp_pred1.squeeze()
        # wtp1 = wtp1.unsqueeze(1) 

        # print("wtp_pred1 shape:", wtp_pred1.shape)
        # print("wtp1 shape:", wtp1.shape)

        # print("Min value in wtp1:", wtp1.min().item())
        # print("Max value in wtp1:", wtp1.max().item())

        criterion = nn.CrossEntropyLoss()
        mse_loss1 = criterion(wtp_pred1.cuda(),wtp1.long().cuda())
        ...
        # print(wtp_pred2.shape, wtp2.shape)
        ...
        rsq = 0
        rsq = torch.tensor(rsq).to(latent_sample.device)
