...
def load_model(directory, is_gpu=True, filename=MODEL_FILENAME, model_type="m3", sup_signal1="xi_fe", sup_signal2="price", sup_signal3="mpg", sup_signal4="hpwt", sup_signal5="space", threshold_val=1):
    ...
    path_to_model = os.path.join(directory, filename)
    model = _get_model(img_size, latent_dim, device, path_to_model,model_type,sup_signal1,sup_signal2,sup_signal3,sup_signal4,sup_signal5,threshold_val)
    return model

def _get_model(img_size, latent_dim, device, path_to_model,model_type,sup_signal1,sup_signal2,sup_signal3,sup_signal4,sup_signal5,threshold_val):
    """ Load a single model.
    ...
    """
    model = VAE(img_size,Encoder,Decoder,WTPregression,latent_dim,model_type,threshold_val,sup_signal1,sup_signal2,sup_signal3,sup_signal4,sup_signal5).to(device)
