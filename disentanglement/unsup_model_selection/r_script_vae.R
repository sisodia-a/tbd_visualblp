...
df <- expand.grid(s)
colnames(df) <- c("s")
df$train_losses_file <- paste0(ARGUMENT,"_s",df$s,"_train_losses.csv")
df$mean_params_train <- paste0(ARGUMENT,"_s",df$s,"_mean_params_train.csv")
df$mean_params_validation <- paste0(ARGUMENT,"_s",df$s,"_mean_params_validation.csv")
df$filename_train <- paste0(ARGUMENT,"_s",df$s,"_filename_train.csv")
df$filename_validation <- paste0(ARGUMENT,"_s",df$s,"_filename_validation.csv")
...
print(dim(total))
write.csv(total,paste0(ARGUMENT,"_s",df$s[i],"_total.csv"),row.names=FALSE)
}

df$total <- paste0(ARGUMENT,"_s",df$s,"_total.csv")
for(i in 1:nrow(df))
{
